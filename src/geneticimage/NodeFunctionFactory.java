...
public class NodeFunctionFactory {
    ...
    private Context getContext(Class contextClass) {
        ...
        if (context == null) {
            try {
                ...
                throw new RuntimeException("Problem with context??", ex);
    ...
    public float getNFClassWeight(Class<? extends Context> contextClass, Class<? extends NodeFunction> nfClass) {
        ...
        // Use the library
        return NodeFunctionLibrarySettings.getInstance().getWeight(nfClass);
    ...
    public int getNumberNBNodes() {
        return numberNBNodes;
    }

    public Node makeTree(Class baseContextClass, Class outputClass) {
        return makeTree(baseContextClass, outputClass, 0);
    }
    ...
    /**
     * Returns true if there are functions which will produce output that is as input by this NF.
     * This only searches the surface level, though.
     * @param nf
     * @return
     */
    private boolean canSupportInput(Class contextClass, NodeFunction nf) {
        if (nf.getInputClasses() != null) {
            for (int i = 0; i < nf.getInputClasses().length; i++) {
                if (getMatches(contextClass, nf.getInputClasses()[i], true).isEmpty()) {
                    return false;
                }
            }
        }
        ...
        if (nf instanceof NodeFunction_NodeBase) {
            NodeFunction_NodeBase nfnb = (NodeFunction_NodeBase) nf;
            for (int i = 0; i < nfnb.getNumberSubnodes(); i++) {

                if (getMatches(contextClass, nfnb.getNodeBaseClass(i), true).isEmpty()) {
                    return false;
                }
            }
        }
    ...
    public List<Class<? extends NodeFunction>> getMatches(Class contextClass, Class outputClass, boolean seekTerminal) {
        ...
        for (NodeFunction nf : allFunctions) {
            if (nf.getReturnClass() == outputClass &&
                    ...
                    getNFClassWeight(contextClass, nf.getClass()) > 0 &&
                    (!seekTerminal || (nf.getInputClasses() == null && !(nf instanceof NodeFunction_NodeBase))) &&
                    canSupportInput(contextClass, nf)) {
    ...
    public NodeFunction randomByOutput(Class contextClass, Class outputClass, boolean seekTerminal) {
        ...
        Context context = getContext(contextClass);
        List<Class<? extends NodeFunction>> matches = getMatches(contextClass, outputClass, seekTerminal);
        /*Context context = getContext(contextClass);
